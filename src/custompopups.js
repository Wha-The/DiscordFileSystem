  ...
  window.open = function(url, name, specs) {
    ...
    if (specs.initialX || specs.initialY) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => popup.style.transition = "box-shadow 0.2s ease 0s", 250)
      }, 1)
    }
    ...
    topBar.addEventListener('mousedown', function(event) {
      if (resizeEdge||isFullScreen) return
      isDragging = true;
    ...
    topBar.addEventListener("dblclick", function(event) {
      ...
      popup.style.transition = "all .35s"
      if(isFullScreen){
        popup.style.transition = "all .35s"
        lastWindowProps = {
        ...
        popup.style.width = (window.innerWidth - 12) + "px"
        popup.style.height = window.innerHeight + "px"
      } else {
        popup.style.top = lastWindowProps.top + "px"
        popup.style.left = lastWindowProps.left + "px"
        popup.style.width = lastWindowProps.width + "px"
        popup.style.height = lastWindowProps.height + "px"
        lastWindowProps = null
      }
    ...
    document.addEventListener('mouseup', function(event) {
      if(isDragging) if(iframe.contentWindow)iframe.contentWindow.focus()
      isDragging = false;
    ...
    window.addEventListener("blur", () => {
      popup.style.boxShadow = "2px 2px 20px rgba(0,0,0,.7)"
    })
    popup.style.boxShadow = "2px 2px 20px rgba(0,0,0,.7)"
    window.addEventListener("focus", () => {
      popup.style.boxShadow = "2px 2px 5px rgba(0,0,0,.7)"
    })
